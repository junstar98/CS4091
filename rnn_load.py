...
x_tr = list(train_data['inputs'])
y_tr = list(train_data['targets'])
...
# Define the model
model = keras.models.load_model('basic_learning_model')
...
encoder_model = keras.models.load_model('basic_encoder_model')
# decoder inference
...
decoder_model = keras.models.load_model('basic_decoder_model')
...
for i in range(len(x_val)):
  ...
  if(i == 21):
      print(x_val[i])
      continue
  tmp_score = nltk.translate.bleu_score.sentence_bleu([seq2summary(y_val[i]).strip().split()], decode_sequence(x_val[i].reshape(1, max_len_text)).strip().split())
  bleu_score += tmp_score

bleu_score /= len(x_val)
bleu_score *= 100;
